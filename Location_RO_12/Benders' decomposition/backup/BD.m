...
%    update constraint_master
    if iteration~=0 
        % redefine re-allocation decision variable
        W{iteration} = sdpvar(ni,nj,'full'); 
        L2{iteration} = sdpvar(1);        
        % cutting plane
        sum_yf = 0;sum_kg = 0;sum_h = 0;
        for i = 1:ni
           for j = 1:nj
              sum_yf = sum_yf +  y(j)*f_new(i,j);
              sum_kg = sum_kg + (1-K{kr(iteration)}(j))*g_new(i,j);
           end
           sum_h = sum_h + h_new(i);
        end
        constraints_master =[constraints_master,eta >= (sum_yf + sum_kg + sum_h)];
        %
  ...
  time = time+t1;
    ...
    if sol.problem == 0
         ...
         f_new = value(f{kr_new}); g_new = value(g{kr_new}); h_new = value(h{kr_new});
    else
         display('Something went wrong!');
         sol.info
         yalmiperror(sol.problem)
