...
% Options
options = sdpsettings('solver','gurobi','verbose',0);%,'showprogress',1);
options1 = sdpsettings('solver','gurobi','verbose',2,'showprogress',1);
...
%         end
  ...
  filename = ['model_MP_iteration_',num2str(iteration),'.mat'];
  %[model]=export(constraints_master,objective_master,sdpsettings('solver','gurobi'));
  %save(filename,'model')
  Time_MP = toc(t1)
  time = time+t1;
    if sol.problem == 0
    ...
    % solve SP
    t2 = tic;
    sol = optimize(constraints_sub,objective_sub,options);
    filename = ['model_SP_iteration_',num2str(iteration),'.mat'];
    %[model]=export(constraints_sub,objective_sub,sdpsettings('solver','gurobi'));
    %save(filename,'model')
    Time_SP = toc(t2)
    time = time + t2;
    if sol.problem == 0
