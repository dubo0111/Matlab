...
for i=1:5%
   ni=ex_size(i);
    ...
    for k=1:Np%
        p=p_now(k);
        % Nex*Np groups of data
        data=zeros(Nk,4); % save final data for file
        for l=1:Nk%
            num_k=k_size(l);
            data_k=zeros(Nex2*Na,3); %save all Nex2*Na (15) data
            index_data_k=1;
            for j=1:Nex2
                for m=1:Na
                    a1=a1_all(m);
            ...
            data(l,:)=[num_k,sum(data_k)/(Nex2*Na)];% mean value for k
        end
        %write data
        filename=[num2str(ni),'_',num2str(p),'.dat'];
