function [cdx1,cdw1,L1_1,L2_1,value_x,value_y ] = center1(  )
...
for nk = 1:num_k
    ...
    [col_L2,~] = size(obj);obj(col_L2)=small_num;
    cost_L2=[cost_L2,col_L2];
...
clear model
%% make sure closest facility !!!!!
cx=zeros(ni,nj);
for i = 1:ni
    for j=1:nj
        cx(i,j)=small_num*CD(i,j);
    end
end
cx=reshape(cx',ni*nj,1);
obj(1:ni*nj)=cx;
%% 
...
for nk = 1:num_k
    ...
    [cost,facility]=min(CD1{nk},[],2); %closest faclity
    cdw(nk)=sum(cost);
end
